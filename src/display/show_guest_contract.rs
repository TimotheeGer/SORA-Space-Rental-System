...
pub async fn show_guest_contract() {
    ...
    loop {
        ...
        match contract_service.find_by_guest_id(user.id.clone()).await {
            Ok(contracts) => {
                if contracts.len() == 0 {
                    ...
                    for contract in contracts {
                        let user_host = match user_service.find_by_id(&contract.host_id.clone()).await {
                            Ok(host) => host,
                            Err(_) => {
                                println!("Erreur lors de la récupération de l'hôte");
                                break;
                            }
                        };
                        ...
                        display_contract(&contract, &office, user_host, user.clone());
                    }
                    println!()
                }
            },
